module tokentrip_auctions::auctions {
    ...
    use tokentrip_experiences::experience_nft::{ExperienceNFT, RoyaltyConfig, royalties, royalty_recipient, royalty_basis_points};    use tokentrip_dao::dao::{DAOTreasury, deposit_to_treasury};   
  ...
  /// Inicia una nueva subasta para un ExperienceNFT
   public entry fun create_auction(
    ...
    public entry fun create_tkt_auction(
        ...
        let auction = Auction {
            ...
            end_timestamp_ms: start_time + duration_ms,
            is_settled: false,
        };
   ...
   /// Permite a un usuario realizar una puja en SUI.
    public entry fun place_bid(
        ...
        internal_place_bid(auction, bid_amount, &mut auction.bid_vault, payment_balance, clock, ctx);
    }
    ...
    public entry fun place_bid_tkt(
        ...
        let bid_amount = coin::value(&payment);
        let payment_balance = coin::into_balance(payment);
        internal_place_bid(auction, bid_amount, &mut auction.tkt_bid_vault, payment_balance, clock, ctx);
    }

    // --- Función interna para la lógica de puja ---
   fun internal_place_bid<C>(
        auction: &mut Auction,
        bid_amount: u64,
        bid_vault: &mut Balance<C>,
        payment_balance: Balance<C>,
        clock: &Clock,
        ctx: &mut TxContext
    ) {
        ...
        // --- LÓGICA DE INTERCAMBIO CORREGIDA Y FINAL ---
        let previous_bid_balance = balance::split(bid_vault, balance::value(bid_vault));
        balance::join(bid_vault, payment_balance);
        
        if (option::is_some(&auction.highest_bidder)) {
            // 1. Se extrae el postor anterior. `highest_bidder` queda como `None`.
            let previous_bidder = option::extract(&mut auction.highest_bidder);

            // 2. Se le devuelve su puja.
            if (balance::value(&previous_bid_balance) > 0) {
                transfer::public_transfer(coin::from_balance(previous_bid_balance, ctx), previous_bidder);
            } else {
                balance::destroy_zero(previous_bid_balance);
            };
        } else {
            // Si no había postor, el balance anterior era cero y se destruye.
            balance::destroy_zero(previous_bid_balance);
        };

        // 3. Ahora que `highest_bidder` es `None`, se inserta el nuevo postor.
   ...
   /// Liquida una subasta finalizada en SUI
    public entry fun settle_sui_auction(
        ...
        let auction_id = object::id_from_uid(&id);
    ...
    public entry fun settle_tkt_auction(
        ...
        if (royalty_amount > 0) {
            let royalty_payment = coin::from_balance(balance::split(&mut payment_balance, royalty_amount), ctx);
            transfer::public_transfer(royalty_payment, royalty_config.recipient);
        };
